# drag_drop_runner_unified_fixed.py
...
class PythonFileRunner:
    ...
    def setup_ui(self):
        """Crea l'interfaccia utente avanzata"""
        ...
        self.console.tag_config('stderr', foreground='#ff9ff3')
        
        y_scroll = ttk.Scrollbar(console_frame, orient=tk.VERTICAL, command=self.console.yview)
        x_scroll = ttk.Scrollbar(console_frame, orient=tk.HORIZONTAL, command=self.console.xview)
    ...
    def setup_drag_drop(self):
        """Configura il drag & drop se supportato"""
        try:
            if sys.platform == 'win32':
                from tkinterdnd2 import TkinterDnD
                try:
                    self.root = TkinterDnD.Tk()
                    self.setup_ui()  # Ricrea l'interfaccia con la nuova root
                ...
                try:
                    ...
                    self.log_output("⚠ Drag&Drop non disponibile (usa il pulsante 'Apri file')", 'warning')
        except ImportError:
    ...
    def on_drop(self, event):
        ...
        files = []
        if sys.platform == 'win32':
            # Correzione della sintassi qui - parentesi quadre corrette
    ...
    def execute_python_file(self, file_path):
        ...
        try:
            # Usiamo Popen con stderr reindirizzato a stdout
            ...
            # Leggiamo sia stdout che stderr (uniti)
            while True:
                output = self.process.stdout.readline()
                if output == '' and self.process.poll() is not None:
                    break
                if output:
                    # Identifichiamo automaticamente gli errori
