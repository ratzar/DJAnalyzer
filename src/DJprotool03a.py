...
class DJProToolApp(tk.Tk):
    def __init__(self):
        ...
        tk.Button(ctrl, text="Voice Start", command=self.run_voice_cue).pack(side=tk.LEFT, padx=5)
    ...
    def run_voice_cue(self):
        if not self.path:
            append_output(self.txt, "Seleziona prima file o cartella.")
            return
        append_output(self.txt, "Rilevamento inizio voce in corso…")
        files = [self.path] if os.path.isfile(self.path) else [os.path.join(self.path, f)
                 for f in os.listdir(self.path) if f.lower().endswith(('.mp3', '.wav'))]
        for idx, fpath in enumerate(files, 1):
            name = os.path.basename(fpath)
            try:
                t0 = rileva_inizio_voce(fpath)
                append_output(self.txt, f"{idx}. {name} → Voice Start: {t0:.2f}s")
            except Exception as e:
                logger.error(f"Voice start error {name}: {e}")
                append_output(self.txt, f"Errore rilevamento voce su {name}")
