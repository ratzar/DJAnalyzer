...
class DJAnalyzerApp:
    def __init__(self, master):
        self.master = master
        master.title("DJAnalyzer v0.6.8 - Corretto") # Versione aggiornata
        ...
        # ***** INIZIO CORREZIONE AttributeError *****
        self.select_input_button = ttk.Button(folder_frame, text="Cartella Input", command=self.select_input_folder)
        self.select_input_button.grid(row=0, column=0, padx=5, pady=5)

        ttk.Entry(folder_frame, textvariable=self.input_folder_path, width=50).grid(row=0, column=1, padx=5, pady=5)

        self.select_output_button = ttk.Button(folder_frame, text="Cartella Output", command=self.select_output_folder)
        self.select_output_button.grid(row=1, column=0, padx=5, pady=5)
        
        ttk.Entry(folder_frame, textvariable=self.output_folder_path, width=50).grid(row=1, column=1, padx=5, pady=5)
        # ***** FINE CORREZIONE AttributeError *****
    ...
    def start_analysis(self):
        ...
        self.select_input_button.config(state=tk.DISABLED)   # Ora questo non dovrebbe dare errore
        self.select_output_button.config(state=tk.DISABLED) # Ora questo non dovrebbe dare errore
    ...
    def _worker_thread(self, files_to_analyze):
        ...
        try:
            for i, file_path in enumerate(files_to_analyze):
                ...
                self.analysis_paused_event.wait() 
    ...
    def analyze_file_and_queue_results(self, file_path):
        ...
        try:
            ...
            scaled_energy, energy_color_name = self.analyze_energy(y, sr)
    ...
    def analyze_energy(self, y_audio, sr_audio): 
        try:
            ...
            scaled_value = 1 
            if energy_score > 0: 
                min_expected_score = 0.005 
                max_expected_score = 0.15   
            ...
            scaled_value = int(np.clip(round(scaled_value), 1, 10))
            print(f"DEBUG ENERGIA: Scaled Energy = {scaled_value}")
            color_name = ENERGY_COLORS.get(scaled_value, "grey")
            return scaled_value, color_name
        except Exception as e:
            print(f"Errore nell'analisi energia: {e}")
            traceback.print_exc()
            return 1, ENERGY_COLORS.get(1, "grey")
    ...
    def reset_ui_after_analysis(self):
        ...
        self.select_input_button.config(state=tk.NORMAL)   # Ora questo non dovrebbe dare errore
        self.select_output_button.config(state=tk.NORMAL) # Ora questo non dovrebbe dare errore
        self.is_paused = False
        self.analysis_thread = None 
        ...
        while not self.result_queue.empty():
            try:
                ...
                break
        if count > 0: print(f"DEBUG: Rimossi {count} messaggi residui dalla coda durante il reset.")
        else: print("DEBUG: Coda già vuota durante il reset finale.")
    ...
    def toggle_pause(self):
        if self.is_paused:
            ...
            self.is_paused = True
            self.analysis_paused_event.clear() 
    ...
    def save_results_to_csv(self):
        ...
        # Assicura che tutte le colonne esistano prima di riordinare
...
if __name__ == "__main__":
    # Workaround per FutureWarning di librosa se necessario a livello globale
    # import warnings
    # warnings.filterwarnings("ignore", category=FutureWarning, module="librosa.core.spectrum")
    # warnings.filterwarnings("ignore", category=FutureWarning, module="librosa.beat")


    
from KAIKeyOptimizer import detect_key_optimized

def avvia_analisi_ottimizzata():
    file_paths = filedialog.askopenfilenames(filetypes=[("File audio", "*.mp3 *.wav")])
    for file_path in file_paths:
        file_name = os.path.basename(file_path)
        bpm = detect_bpm(file_path)
        key, camelot = detect_key_optimized(file_path)
        energy = calculate_energy(file_path)
        compatibili = get_compatible_keys(camelot)

        tree.insert("", "end", values=(file_name, bpm, key, camelot, ", ".join(compatibili), energy))

    salva_csv("DJAnalyzer_analisi_ottimizzata.csv")
